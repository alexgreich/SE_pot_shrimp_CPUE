...
#global model
m_glob_r <- gam(log(CPUE_nom + 0.001) ~ factor(Season.Ref) + factor(ADFG.Number)+ s(vessel_count_mgmt_u, k=4) + s(jdate, k=4) + s(Analysis.Area, bs="re", k=4), data=D7, ) #remove jdate? #crap should ADFG.Number be a random effect too?
...
test2 <- gam(log(CPUE_nom+0.001) ~ vessel_count_mgmt_u + Season.Ref + ADFG.Number + s(jdate, k=4) + s(Analysis.Area, bs="re"), data=(D7)) #that did not work. Soemthign abotu my random effect not working.
```
