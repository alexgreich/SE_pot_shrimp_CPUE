...
#m_interactions
m_glob <- lmer(log(CPUE_nom+0.001) ~ Season.Ref + ADFG.Number+vessel_count_aa + (1|ADFG.Number) (1|Analysis.Area) + (Season.Ref|Analysis.Area), data=D7, REML=F) #takes forever
...
m_glob6 <- lmer(log(CPUE_nom+0.001) ~ Season.Ref + vessel_count_aa + (1|ADFG.Number) + (1|Analysis.Area), data=D7, REML=F)
...
AIC(m_glob2, m_glob3, m_glob4,m_glob5,m_glob6, m_glob7, m_glob8) #says glob 5. Let's chgeck for correlation tho. 6 is better than 3, if we choose ranef
summary(m_glob2)
summary(m_glob4)
