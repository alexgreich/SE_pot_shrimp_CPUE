...
#5/30/24: different vessel modes for the different areas: predict
...
```{r}
#EU_dat<- expand.grid(Season.Ref = unique(D7_filled_3$Season.Ref),
                #              Analysis.Area = "Upper Ernest Sound", 
                 #             ADFG.Number = factor(52131) 
                  #    )
...
D7_pred_comb %>% filter(Analysis.Area =="Zimovia Strait") %>%
  ggplot() + aes(x=Season.Ref, y=mu) + geom_errorbar(aes(ymin=lower, ymax=upper)) + geom_point()+geom_line(aes(group="Analysis.Area"))

pred_dat_biascorrected %>% filter(Analysis.Area =="Zimovia Strait") %>%
  ggplot() + aes(x=Season.Ref, y=mu) + geom_errorbar(aes(ymin=lower, ymax=upper)) + geom_point()+geom_line(aes(group="Analysis.Area")) 

comb_D7 %>% filter(Analysis.Area =="Zimovia Strait") %>%
  ggplot() + aes(x=Season.Ref, y=mu) + geom_errorbar(aes(ymin=lower, ymax=upper)) + geom_point() + geom_line(aes(group="Analysis.Area"))
