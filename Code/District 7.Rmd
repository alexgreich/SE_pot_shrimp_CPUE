...
Not sure if I should aggregate cpue's (sum) or take the average.
Stuck here 6/3/24- I need to take the variance of the mean- either by delta method or bootstrapping.
```{r}
...
D7_results_area_weighted <- D7_pred_comb %>%
  ...
  summarise(CPUE_avg = weighted.mean(x=mu, y=area_sqmi),
            CPUE_sd = sqrt(weighted.mean(x=sigma^2, y=area_sqmi))
  )%>%
...
ggplot(D7_results_area_weighted)  + aes(x=Season.Ref, y=CPUE_avg) + 
   geom_point(size=3)+
   geom_errorbar( aes(ymin=lower, ymax=upper)) +
   geom_line(aes(group=1))+ #that works+
   #geom_hline(aes(yintercept=mean(std_dat_ranef_lim$bt_cpue)), linetype="dashed")+
