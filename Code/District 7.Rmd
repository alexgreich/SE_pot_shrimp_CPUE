...
##### bias corrected: do I use in every case?
...
Ok, based on these graphs I've decided area-wide analysis is fine, and I'd like to use the mode of each vessel for analysis. Now make the graphs for D7 and do the other stuff that Max wanted.
...
make_plots <- function(DATA_PRED, AREA_NAME, DATA_RAW){ #data pred is the
...
#upper ERnest
Upper_ernest_plot <- make_plots(DATA_PRED = ernU_pred, AREA_NAME = "Upper Ernest Sound", DATA_RAW=D7)
Lower_ernest_plot <- make_plots(DATA_PRED = ernL_pred, AREA_NAME = "Lower Ernest Sound", DATA_RAW=D7)
Brad_plot <- make_plots(DATA_PRED = brad_pred, AREA_NAME = "Bradfield Canal", DATA_RAW=D7)
Zim_plot <- make_plots(DATA_PRED = zim_pred, AREA_NAME = "Zimovia Strait", DATA_RAW=D7)
...
D7 total graphs
Read up on combining cpue's from area:
```{r}
#how to use the weighing component... now?
```
...
What do I do about the imputed data? How to report that?
I can plot raw catch and effort, I dont need to standardize that right?
