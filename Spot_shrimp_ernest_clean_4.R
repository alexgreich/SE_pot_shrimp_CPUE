...
##################################################
##Spot shrimp earnest clean 4: Earnest CPUE standardization
#####################################################3
#created 12/04/23 to clarify Exploring_the_data_3.R file
##Ernest sound specific
...
###jdate as covariate, vessel # as factor. jdate smoothed, vessel # as (smoothed?) random variable.
### see Jig_BRF_CPUE and the associated prep code for phil's rockfish example
...
corr_spot2_limited <- corr_spot_limited %>% #a closer look
  select(DOL.Month, Stat.Week, CPUE_nom, Batch.Year,vessel_count, jdate)

#correlation plots
#pairs(corr_spot) #argh it does not like non numeric arguments
pairs(corr_spot2)
pairs(corr_spot2_limited)



#below is actually a pretty good graph showing change in range and median fishing date over the years
ggplot(corr_spot2_limited) + aes(x=Batch.Year, y=jdate, group=Batch.Year, color=CPUE_nom) + geom_jitter() + geom_boxplot(alpha=0.3) + scale_color_viridis() #mean jdate for analysis is....287 or so
ggplot(corr_spot2) + aes(x=Batch.Year, y=jdate, group=Batch.Year, color=CPUE_nom) + geom_jitter() + geom_boxplot(alpha=0.3) + scale_color_viridis()
#not sure how I can use jdate to predict for a new season. Might not be the right move to make...
